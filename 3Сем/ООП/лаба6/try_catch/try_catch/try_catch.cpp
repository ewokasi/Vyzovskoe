...
public:
	...
	NoZero(const char* msg = "ERROR Not neough zeros!!!")
	{
		text = msg;
	}
...
int arr::even_multiplication(int* arr, int size)
{
	...
	if (size <= 1)
	{
		throw std::exception("ERROR Not Enough elements for even position multiplication\n");
	}
	int result = arr[0];
...
int* arr::convert(int* arr, int size)
{
	if (size <= 1)
	{
		throw std::exception("ERROR Not enough elements for convertion\n");
	}
	...
	while (true)
	{
		...
		case 1:
			try
			{
				...
				std::cout <<"sum betwenn zeros "<< list.sum_between_zeros(smth, size) << '\n';
			}
			catch (NoZero& ex)
			{
				std::cout << ex.what() << '\n';
